...
def inference_small_config(x, c):
    ...
    with tf.variable_scope('down_scale1',reuse=c['reuse']):
        ...
        x1 = stack(x1, c)        
        x1 = _max_pool(x1,ksize=2,stride=2)       
    ...
    with tf.variable_scope('down_scale3',reuse=c['reuse']):
       ...
       #     x2 = tf.nn.dropout(x2,keep_prob) 
        ...
        x3 = stack(x2, c)
        x3 = _max_pool(x3,ksize=2,stride=2)
        
        print 'x3:',x3.get_shape()
        
    with tf.variable_scope('down_scale4',reuse=c['reuse']):
        c['bottleneck'] = True
        c['block_filters_internal'] = 32
        c['num_blocks'] = 3
    ...
    with tf.variable_scope('down_scale5',reuse=c['reuse']):
        c['bottleneck'] = True
        c['block_filters_internal'] = 32
        c['num_blocks'] = 3
    ...
    with tf.variable_scope('output',reuse=c['reuse']):
        ...
        c['stride'] = 1
        out = conv(x4, c)
        #out =tf.contrib.layers.fully_connected(x3,c['num_classes'])
        print 'out:',out.get_shape() 
        print 'output:',out.get_shape()
